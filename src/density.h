...
class Density{
public:
    Density(DataSource& dataSource, VpTree& vpTree, int nNearestNeighbours, Progress* pProgress) : _dataSource(dataSource), _vpTree(vpTree), _nNearestNeighbours(nNearestNeighbours), _pProgress(pProgress) {}
    void calculateDensityValues() {
        ...
        for(int i = 0; i < (int)densityVector.size(); i++) {
            ...
            _vpTree.search(numberVector, _nNearestNeighbours, nearestNeighbours);
    ...
    float calculateDensityValue(vector<float>& numberVector) {
        ...
        vector<VpElement> nearestNeighbours;
        if(_vpTree.isBuilt()) {
            _vpTree.search(normalizedNumberVector, _nNearestNeighbours, nearestNeighbours);    
        } else {
            _vpTree.linearSearch(normalizedNumberVector, _nNearestNeighbours, nearestNeighbours);    
        }
        float d = calculateDensityValue(nearestNeighbours);
